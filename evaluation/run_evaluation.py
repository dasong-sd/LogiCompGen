...
from evaluation.config import SCENARIO_CONFIG, load_filtered_ltl_rules
from evaluation.evaluator import StateEvaluator
...
RESULTS_OUTPUT_DIR = Path("results/raw_evaluations_v2")
...
def run_evaluation(scenario: str, model_under_test: str, config_paths: dict):
    ...
    SYSTEM_PROMPT = f"""You are an expert Python developer writing a script for a stateful API environment.
        ...
        **Preprocessed Policy Statements:**
        {policy_context}
        ...
        **Instructions:**
        1. Determine whether the supplied policy statements require additional operations for the current task, and include those operations when applicable.
        2. Use the exact variables defined in the "Initial User Variables" block (e.g., `user_variable_0`). Do not hardcode values from the prompt description.
        3. Wrap the generated program in a markdown code block (```python ... ```). Do not include text outside this block.
    """
    ...
    for i, prompt_data in enumerate(tqdm(valid_prompts, desc=f"Evaluating {scenario}")): 
        ...
        for prompt_type in modes_to_run:
            ...
            evaluation_results.append({
                ...
                "evaluation_protocol_version": "logicompgen-eval-v2",
    ...
    result_path_json = result_dir / result_filename_json
    
    try:
        with result_path_json.open("w", encoding="utf-8") as f:
        ...
        with (result_dir / result_filename_pkl).open("wb") as f:
    ...
    summary_path = result_dir / f"summary_{scenario}_{safe_model}_combined_{timestamp}.json"
